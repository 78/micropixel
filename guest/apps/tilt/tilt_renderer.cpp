...
void TiltGame::InitializeScene() {
    ...
    root_container_ =
        scene_
            .CreateContainer({.clip = {0, 0, static_cast<int32_t>(kScreenWidth), static_cast<int32_t>(kScreenHeight)},
                              .translation = content_offset})
            .value();
    game_container_ = root_container_.CreateContainer({.clip = kBoardRect, .z_order = 0}).value();
    hud_container_ =
        root_container_.CreateContainer({.clip = {0, 0, static_cast<int32_t>(kScreenWidth), kHudHeight}, .z_order = 10})
            .value();
    modal_container_ =
        root_container_
            .CreateContainer({.clip = {0, 0, static_cast<int32_t>(kScreenWidth), static_cast<int32_t>(kScreenHeight)},
    ...
    board_base_node_ = game_container_
                           .CreateSprite(board_base_texture_, kBoardRect,
                                         {0, 0, static_cast<int32_t>(board_base_texture_.width()),
                                          static_cast<int32_t>(board_base_texture_.height())})
                           .value();
    board_tile_batch_ = game_container_.CreateSpriteBatch(board_tile_texture_, kBoardTileBatchCapacity).value();
    board_frame_node_ = game_container_
                            .CreateSprite(board_frame_texture_, kBoardRect,
                                          {0, 0, static_cast<int32_t>(board_frame_texture_.width()),
                                           static_cast<int32_t>(board_frame_texture_.height())})
                            .value();
    object_batch_ = game_container_.CreateSpriteBatch(object_texture_, kObjectBatchCapacity).value();
    fan_batch_ = game_container_.CreateSpriteBatch(fan_texture_, kMaximumFanCount).value();
    moving_wall_node_ = game_container_
                            .CreateSprite(mechanic_texture_,
                                          {0, 0, static_cast<int32_t>(kObjectFrameLogicalPixels),
                                           static_cast<int32_t>(kObjectFrameLogicalPixels)},
                                          MechanicFrameSource(MechanicFrame::kBlockerHorizontal))
                            .value();
    gate_node_ = game_container_
                     .CreateSprite(mechanic_texture_,
                                   {0, 0, static_cast<int32_t>(kObjectFrameLogicalPixels),
                                    static_cast<int32_t>(kObjectFrameLogicalPixels)},
                                   MechanicFrameSource(MechanicFrame::kGateHorizontal))
                     .value();
    pressure_gate_node_ = game_container_
                              .CreateSprite(mechanic_texture_,
                                            {0, 0, static_cast<int32_t>(kObjectFrameLogicalPixels),
                                             static_cast<int32_t>(kObjectFrameLogicalPixels)},
                                            MechanicFrameSource(MechanicFrame::kGateHorizontal))
                              .value();
    pressure_plate_node_ = game_container_
                               .CreateSprite(mechanic_texture_,
                                             {0, 0, static_cast<int32_t>(kObjectFrameLogicalPixels),
                                              static_cast<int32_t>(kObjectFrameLogicalPixels)},
                                             MechanicFrameSource(MechanicFrame::kPressurePlateOff))
                               .value();
    portal_batch_ = game_container_.CreateSpriteBatch(mechanic_texture_, 2U).value();
    trail_batch_ = game_container_.CreateSpriteBatch(kTrailCapacity).value();
    marble_node_ =
        game_container_
            .CreateSprite(object_texture_, ObjectDestination(model_.ball()), FrameSource(ObjectFrame::kMarble))
            .value();
    goal_node_ = game_container_
                     .CreateSprite(object_texture_,
                                   ObjectDestination({static_cast<float>(model_.level().goal.x),
                                                      static_cast<float>(model_.level().goal.y)}),
                                   FrameSource(ObjectFrame::kGoal0))
                     .value();
    particle_batch_ = game_container_.CreateSpriteBatch(kParticleCapacity).value();

    const int32_t safe_left = static_cast<int32_t>(renderer_info_.safe_area_insets().left);
    title_node_ = hud_container_
                      .CreateSprite(title_texture_, {safe_left + 20, 12, 240, 60},
                                    {0, 0, static_cast<int32_t>(title_texture_.width()),
                                     static_cast<int32_t>(title_texture_.height())})
                      .value();
    star_panel_ = hud_container_
                      .CreateRoundedRect({312, 12, 174, 60}, {.fill = micropixel::Color::Rgb(8U, 18U, 28U),
                                                              .stroke = micropixel::Color::Rgb(35U, 62U, 82U),
                                                              .radius = 28,
                                                              .stroke_width = 3})
                      .value();
    hud_star_batch_ = hud_container_.CreateSpriteBatch(hud_texture_, kStarCount).value();
    tilt_panel_ = hud_container_
                      .CreateRoundedRect({626, 10, 76, 64}, {.fill = micropixel::Color::Rgb(8U, 18U, 28U),
                                                             .stroke = micropixel::Color::Rgb(35U, 62U, 82U),
                                                             .radius = 30,
                                                             .stroke_width = 3})
                      .value();
    tilt_icon_node_ = hud_container_.CreateSprite(hud_texture_, {634, 12, 60, 60}, HudFrameSource(2U)).value();
    level_label_ =
        hud_container_
            .CreateLabel({550, 9}, "LEVEL 001", micropixel::Color::White(), micropixel::SystemFont::kSmall, true)
            .value();
    time_label_ = hud_container_
                      .CreateLabel({550, 39}, "00:00", micropixel::Color::Rgb(45U, 226U, 230U),
                                   micropixel::SystemFont::kMedium, true)
                      .value();

    modal_dim_ = modal_container_.CreateShape(kBoardRect, micropixel::Color::Black(), 176U).value();
    modal_title_ =
        modal_container_
            .CreateLabel({360, 286}, "JUICY TILT", micropixel::Color::White(), micropixel::SystemFont::kLarge, true)
            .value();
    modal_subtitle_ = modal_container_
                          .CreateLabel({360, 650}, "TAP LOGO TO PAUSE", micropixel::Color::Rgb(130U, 158U, 178U),
...
void TiltGame::RenderObjects() {
    if (rendered_level_index_ != model_.level_index()) {
        ...
        board_tile_batch_.SetInstance(kIceTileInstance, {.destination = BoardIceDestination(level.ice),
        ...
        for (uint16_t index = 0U; index < kMaximumFanCount; ++index) {
            ...
            board_tile_batch_.SetInstance(static_cast<uint16_t>(kAirflowTileInstanceOffset + index),
        ...
        for (uint32_t wall_index = 0U; wall_index < level.visual_wall_count; ++wall_index) {
            ...
            for (uint32_t block_index = 0U; block_index < block_count; ++block_index) {
                ...
                board_tile_batch_.SetInstance(static_cast<uint16_t>(kWallTileInstanceOffset + wall_block_index),
    ...
    for (uint32_t index = 0U; index < kStarCount; ++index) {
        object_batch_.SetInstance(static_cast<uint16_t>(kStarInstanceOffset + index),
    ...
    object_batch_.SetInstance(
        kPitInstance, {.destination = ObjectDestination(
                           {static_cast<float>(model_.level().pit.x), static_cast<float>(model_.level().pit.y)}),
    ...
    object_batch_.SetInstance(kBumperInstance, {.destination = ObjectDestination(model_.bumper_position()),
    ...
    for (uint16_t index = 0U; index < kMaximumFanCount; ++index) {
        ...
        fan_batch_.SetInstance(
            index, {.destination = ObjectDestination({static_cast<float>(fan.x), static_cast<float>(fan.y)}),
    ...
    moving_wall_node_.SetVisible(moving_wall_visible);
    if (moving_wall_visible) {
        moving_wall_node_.SetDestination(MechanicDestination(moving_wall));
        moving_wall_node_.SetSource(MechanicFrameSource(moving_wall.width >= moving_wall.height
                                                            ? MechanicFrame::kBlockerHorizontal
    ...
    gate_node_.SetVisible(gate_visible);
    if (gate_visible) {
        gate_node_.SetDestination(MechanicDestination(gate));
        gate_node_.SetSource(MechanicFrameSource(gate.width >= gate.height ? MechanicFrame::kGateHorizontal
                                                                           : MechanicFrame::kGateVertical));
        gate_node_.SetOpacity(model_.gate_open() ? 48U : 255U);
    }
    ...
    pressure_plate_node_.SetVisible(pressure_visible);
    pressure_gate_node_.SetVisible(pressure_gate.gate.width != 0 && pressure_gate.gate.height != 0);
    if (pressure_visible) {
        constexpr int32_t plate_size = 72;
        pressure_plate_node_.SetDestination(
            ClampToViewport({kBoardX + pressure_gate.plate.x - plate_size / 2,
                             kBoardY + pressure_gate.plate.y - plate_size / 2, plate_size, plate_size}));
        pressure_plate_node_.SetSource(MechanicFrameSource(
            model_.pressure_gate_open() ? MechanicFrame::kPressurePlateOn : MechanicFrame::kPressurePlateOff));
        pressure_gate_node_.SetDestination(MechanicDestination(pressure_gate.gate));
        pressure_gate_node_.SetSource(MechanicFrameSource(pressure_gate.gate.width >= pressure_gate.gate.height
                                                              ? MechanicFrame::kGateHorizontal
                                                              : MechanicFrame::kGateVertical));
        pressure_gate_node_.SetOpacity(model_.pressure_gate_open() ? 38U : 255U);
    }
    ...
    for (uint16_t index = 0U; index < 2U; ++index) {
        ...
        portal_batch_.SetInstance(
            index, {.destination = ClampToViewport({kBoardX + portal.x - portal_size / 2,
                                                    kBoardY + portal.y - portal_size / 2, portal_size, portal_size}),
    ...
    goal_node_.SetSource(
        FrameSource(static_cast<ObjectFrame>(static_cast<uint32_t>(ObjectFrame::kGoal0) + animation_phase)));
    goal_node_.SetDestination(
        ObjectDestination({static_cast<float>(model_.level().goal.x), static_cast<float>(model_.level().goal.y)}));
    goal_node_.SetOpacity(model_.goal_unlocked() ? 255U : 92U);
    marble_node_.SetDestination(ObjectDestination(model_.ball()));
}

void TiltGame::RenderEffects() {
    ...
    for (uint16_t index = 0U; index < kTrailCapacity; ++index) {
        ...
        if (!trail.active) {
            trail_batch_.SetInstanceVisible(index, false);
            continue;
        }
        ...
        trail_batch_.SetInstance(
            index, {.destination = destination, .color = color, .opacity = 255U, .visible = !destination.empty()});
    }
    for (uint16_t index = 0U; index < kParticleCapacity; ++index) {
        const Particle& particle = particles_[index];
        if (!particle.active) {
            particle_batch_.SetInstanceVisible(index, false);
            continue;
        }
...
void TiltGame::RenderHud() {
    const Vec2 tilt = input_.tilt();
    tilt_icon_node_.SetDestination({634 + RoundToInt(tilt.x * 3.0F), 12 + RoundToInt(tilt.y * 3.0F), 60, 60});
    for (uint16_t index = 0U; index < kStarCount; ++index) {
        hud_star_batch_.SetInstance(index, {.destination = {321 + static_cast<int32_t>(index) * 54, 18, 48, 48},
    ...
    level_label_.SetText(level.c_str());
}

void TiltGame::RenderModal() {
    const bool visible = screen_ != Screen::kPlaying && screen_ != Screen::kCalibrating;
    modal_container_.SetVisible(visible);
    if (!visible) {
        return;
    }
    ...
    previous_level_button_.SetVisible(has_previous_level);
    next_level_button_.SetVisible(has_next_level);
    previous_level_button_.SetEnabled(has_previous_level);
    next_level_button_.SetEnabled(has_next_level);
    action_button_.SetVisible(true);
    if (screen_ == Screen::kMenu) {
        const Line selected_level = FormatLevelLabel(selected_level_index_);
        modal_title_.SetText(selected_level.c_str());
        modal_title_.SetColor(micropixel::Color::White());
        modal_subtitle_.SetPosition({360, 650});
        modal_subtitle_.SetText("TAP LOGO TO PAUSE");
        micropixel::Assert(action_button_.SetText("START").has_value(), "tilt: START label failed");
        secondary_button_.SetVisible(selected_level_index_ != 0U);
        micropixel::Assert(secondary_button_.SetText("RUN FROM 01").has_value(), "tilt: run-from-first label failed");
    } else if (screen_ == Screen::kPaused) {
        modal_title_.SetText("PAUSED");
        modal_title_.SetColor(micropixel::Color::White());
        modal_subtitle_.SetPosition({360, 350});
        modal_subtitle_.SetText("TIMER AND HAZARDS FROZEN");
        micropixel::Assert(action_button_.SetText("RESUME").has_value(), "tilt: RESUME label failed");
        secondary_button_.SetVisible(true);
        micropixel::Assert(secondary_button_.SetText("RESTART").has_value(), "tilt: RESTART label failed");
    } else if (screen_ == Screen::kComplete) {
        const bool final_level = model_.level_index() + 1U == kLevelCount;
        secondary_button_.SetVisible(final_level);
        modal_title_.SetText(final_level ? "ALL CLEAR" : "LEVEL CLEAR");
        modal_title_.SetColor(micropixel::Color::Rgb(255U, 176U, 32U));
        modal_subtitle_.SetPosition({360, 350});
        char time[6]{};
        FormatTime(model_.elapsed_us(), time);
        Line summary;
        ...
        summary.Append(time);
        modal_subtitle_.SetText(summary.c_str());
        micropixel::Assert(action_button_.SetText(final_level ? "RETRY" : "NEXT").has_value(),
                           "tilt: completion button label failed");
        if (final_level) {
            micropixel::Assert(secondary_button_.SetText("RUN FROM 01").has_value(), "tilt: final replay label failed");
        }
    } else {
        secondary_button_.SetVisible(false);
        modal_title_.SetText("SENSOR REQUIRED");
        modal_title_.SetColor(micropixel::Color::Rgb(251U, 113U, 133U));
        modal_subtitle_.SetPosition({360, 350});
        modal_subtitle_.SetText("NO ACCELEROMETER FOUND");
        action_button_.SetVisible(false);
    }
    action_button_.Sync();
    secondary_button_.Sync();
    previous_level_button_.Sync();
    next_level_button_.Sync();
}
