...
GpioBackend::GpioBackend() {
    for (uint32_t index = 0U; index < lines_.size(); ++index) {
        lines_[index].owner = this;
        lines_[index].line_number = peripheral_ids::kApplicationGpioLines[index];
        lines_[index].device = peripheral_ids::Gpio(peripheral_ids::kApplicationGpioLines[index]);
        lines_[index].pin = static_cast<gpio_num_t>(peripheral_ids::kApplicationGpioLines[index]);
    }
...
esp_err_t GpioBackend::Initialize() {
    ESP_LOGI(kTag, "%u application GPIO lines ready; edge resources are lazy", static_cast<unsigned>(lines_.size()));
    return ESP_OK;
}

int32_t GpioBackend::GetInfo(micropixel_device_id_t device, micropixel_gpio_info_t& info_out) const {
    const Line* line = Find(device);
    if (line == nullptr) {
...
int32_t GpioBackend::Open(micropixel_device_id_t device, uint16_t mode, uint16_t pull, uint16_t edge,
                          uint32_t initial_value, uint32_t pwm_frequency_hz, device::GpioEdgeSink edge_sink,
                          void* edge_context) {
    Line* line = Find(device);
    if (line == nullptr) {
        return MICROPIXEL_STATUS_NOT_FOUND;
    }
...
int32_t GpioBackend::ConfigureInput(Line& line, uint16_t pull, uint16_t edge, device::GpioEdgeSink edge_sink,
                                    void* edge_context) {
    ...
    if (edge != MICROPIXEL_GPIO_EDGE_NONE) {
        ...
        if (status != ESP_OK) {
            ...
            portENTER_CRITICAL(&state_lock_);
            line = Line{.owner = this, .device = line.device, .pin = line.pin, .line_number = line.line_number};
            portEXIT_CRITICAL(&state_lock_);
...
int32_t GpioBackend::ConfigureOutput(Line& line, bool initial_value) {
    gpio_config_t config{};
    config.pin_bit_mask = 1ULL << static_cast<uint32_t>(line.pin);
...
int32_t GpioBackend::ConfigurePwm(Line& line, uint32_t frequency_hz, uint16_t initial_duty_per_mille) {
...
int32_t GpioBackend::Read(micropixel_device_id_t device, bool& value_out) const {
    const Line* line = Find(device);
...
int32_t GpioBackend::Write(micropixel_device_id_t device, bool value) {
    Line* line = Find(device);
...
int32_t GpioBackend::SetPwmDuty(micropixel_device_id_t device, uint16_t duty_per_mille) {
    Line* line = Find(device);
...
void GpioBackend::Close(micropixel_device_id_t device) {
    Line* line = Find(device);
    if (line == nullptr || !line->active) {
        return;
    }
    ...
    const micropixel_device_id_t line_device = line->device;
    const gpio_num_t pin = line->pin;
    const uint16_t number = line->line_number;
    *line = Line{.owner = this, .device = line_device, .pin = pin, .line_number = number};
...
void GpioBackend::SuspendEvents() {
...
int32_t GpioBackend::ResumeEvents() {
    if (!events_suspended_) {
        return MICROPIXEL_STATUS_OK;
    }
    if (edge_line_count_ == 0U) {
...
GpioBackend::Line* GpioBackend::Find(micropixel_device_id_t device) {
    for (Line& line : lines_) {
...
const GpioBackend::Line* GpioBackend::Find(micropixel_device_id_t device) const {
...
bool GpioBackend::StartEdgeWorker() {
    if (worker_ != nullptr) {
        return true;
    }
...
void GpioBackend::StopEdgeWorker() {
    if (worker_ != nullptr) {
...
void GpioBackend::ReleaseEdgeResources() {
...
void IRAM_ATTR GpioBackend::OnEdge(void* context) {
    auto& line = *static_cast<Line*>(context);
    GpioBackend& backend = *line.owner;
    const uint32_t index = static_cast<uint32_t>(&line - backend.lines_.data());
    const EdgeRecord record{index, static_cast<uint32_t>(gpio_get_level(line.pin)),
                            static_cast<uint64_t>(esp_timer_get_time())};
    BaseType_t higher_priority_woken = pdFALSE;
    if (backend.edge_queue_ != nullptr) {
        (void)xQueueSendFromISR(backend.edge_queue_, &record, &higher_priority_woken);
    }
...
void GpioBackend::WorkerEntry(void* context) {
    static_cast<GpioBackend*>(context)->Worker();
    vTaskDelete(nullptr);
}

void GpioBackend::Worker() {
    while (!stopping_.load(std::memory_order_acquire)) {
        EdgeRecord record{};
        if (xQueueReceive(edge_queue_, &record, portMAX_DELAY) != pdTRUE || record.line_index >= lines_.size()) {
            continue;
        }
        if (callback_mutex_ == nullptr || xSemaphoreTake(callback_mutex_, portMAX_DELAY) != pdTRUE) {
            continue;
        }
        device::GpioEdgeSink sink = nullptr;
        void* sink_context = nullptr;
        micropixel_device_id_t id = 0U;
        portENTER_CRITICAL(&state_lock_);
        ...
        portEXIT_CRITICAL(&state_lock_);
        if (sink != nullptr) {
            sink(sink_context, id, record.value != 0U, record.timestamp_us);
        }
        (void)xSemaphoreGive(callback_mutex_);
    }
    if (worker_stopped_ != nullptr) {
