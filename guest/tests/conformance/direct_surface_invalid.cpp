...
micropixel_surface_present_request_t ValidPresent(micropixel_surface_handle_t surface, uint32_t width,
    ...
    request.surface_handle = surface;
...
int main() {
    ...
    create.buffer_count = info.max_surface_buffers + 1U;
    ...
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_CREATE, &create, sizeof(create), &created, sizeof(created)) !=
        ...
        created.size != sizeof(created) || created.surface_handle == 0U ||
        created.native_pixel_format != MICROPIXEL_PIXEL_FORMAT_RGB565) {
        return 57;
    }
    // Fixed requests reject reserved bits, oversized size fields and trailing bytes.
    struct ExtendedHandle {
        micropixel_handle_request_t request;
        uint32_t extra;
    } malformed{{sizeof(micropixel_handle_request_t), 1U, created.surface_handle}, 0U};
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_DESTROY, &malformed, sizeof(malformed.request), nullptr, 0U) !=
        MICROPIXEL_STATUS_INVALID_ARGUMENT)
        return 80;
    malformed.request.reserved0 = 0U;
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_DESTROY, &malformed, sizeof(malformed), nullptr, 0U) !=
        MICROPIXEL_STATUS_INVALID_ARGUMENT)
        return 81;
    malformed.request.size = sizeof(malformed);
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_DESTROY, &malformed, sizeof(malformed), nullptr, 0U) !=
        MICROPIXEL_STATUS_INVALID_ARGUMENT)
        return 82;
    ...
    present = ValidPresent(created.surface_handle + 1U, info.width, info.height);
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_PRESENT, &present, sizeof(present), nullptr, 0U) !=
        MICROPIXEL_STATUS_NOT_FOUND) {
        return 60;
    }
    // buffer_index beyond buffer_count.
    present = ValidPresent(created.surface_handle, info.width, info.height);
    ...
    present = ValidPresent(created.surface_handle, info.width, info.height);
    present.source_width = info.width - 1U;
    if (Call(MICROPIXEL_GRAPHICS_METHOD_SURFACE_PRESENT, &present, sizeof(present), nullptr, 0U) !=
        MICROPIXEL_STATUS_INVALID_ARGUMENT) {
        return 64;
    }
    // Host buffers are panel-sized: a scaled present has nothing to scale.
    present = ValidPresent(created.surface_handle, info.width / 2U, info.height / 2U);
    ...
    if ((service.capabilities & MICROPIXEL_GRAPHICS_CAP_RASTER) != 0U) {
        struct {
    ...
    micropixel_handle_request_t destroy{static_cast<uint16_t>(sizeof(destroy)), 0U, created.surface_handle};
