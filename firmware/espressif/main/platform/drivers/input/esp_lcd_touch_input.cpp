...
esp_err_t EspLcdTouchInput::Initialize(esp_lcd_touch_handle_t touch, common::I2cExecutor& executor) {
...
esp_err_t EspLcdTouchInput::Start(lv_display_t* display) {
    ...
    const esp_err_t prime_status = executor_->Invoke(common::I2cExecutor::Priority::kHigh, PrimeEntry, this);
...
void IRAM_ATTR EspLcdTouchInput::InterruptEntry(esp_lcd_touch_handle_t) {
    ...
    if (!instance->executor_->PostFromIsr(common::I2cExecutor::Priority::kHigh, ProcessEntry, instance,
...
void EspLcdTouchInput::ProcessInterrupt() {
    ...
    if (interrupts_.load(std::memory_order_acquire) != observed_interrupts) {
        if (!work_pending_.exchange(true, std::memory_order_acq_rel) &&
            !executor_->Post(common::I2cExecutor::Priority::kHigh, ProcessEntry, this)) {
            work_pending_.store(false, std::memory_order_release);
        }
    }
}
