...
int main() {
    ...
    micropixel::AssertThat(info.has_value(), "audio_synth: Audio 1.0 capability missing");
    micropixel::AssertThat(info.value().max_voices == 8U, "audio_synth: expected bounded 8-voice mixer");
    micropixel::Tone invalid = Tone(micropixel::Waveform::kSine, 440U);
    invalid.volume_per_mille = 1001U;
    micropixel::AssertThat(!audio.Play(invalid).has_value(), "audio_synth: invalid volume accepted");
    invalid = Tone(micropixel::Waveform::kSine, 19U);
    micropixel::AssertThat(!audio.Play(invalid).has_value(), "audio_synth: invalid frequency accepted");
    invalid = Tone(micropixel::Waveform::kSine, 440U);
    invalid.attack = 300_ms;
    micropixel::AssertThat(!audio.Play(invalid).has_value(), "audio_synth: invalid envelope accepted");
    ...
    for (const micropixel::Tone& item : sequence) {
        micropixel::AssertThat(audio.Play(item).has_value(), "audio_synth: waveform command rejected");
        WaitFor(app, 340_ms);
    }
    ...
    for (uint32_t frequency_hz : kChord) {
        ...
        micropixel::AssertThat(audio.Play(voice).has_value(), "audio_synth: polyphonic command rejected");
    }
    WaitFor(app, 750_ms);
    micropixel::AssertThat(audio.StopAll().has_value(), "audio_synth: StopAll rejected");
