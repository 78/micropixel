...
| Resource / Atlas | `Resources::Load()`、`ResourceReadyEvent`、`DrawBitmapRegion()`、生成的 atlas metadata | `pages/resource_atlas_demo.cpp` |
...
定位真实用法。Graphics 没有独立页面：`demo_app.cpp` 统一创建和提交 `CommandBuffer`，各功能页直接
向它写入自己的 Graphics 命令。

首页、返回按钮和各页面操作按钮共用 `sdk/ui/button.hpp`：按下时用 `BlendRect()` 叠加半透明反馈，
移出按钮会撤销按下态，只有移回并在按钮内松开才触发动作。这个应用也因此直接覆盖 Graphics
