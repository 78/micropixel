...
`sfx.json` 是 Snake 音效和背景旋律的唯一参数源。构建流程按照当前 Host 的 16 kHz
...
- `event A`：经过 A-weighting 后的整段累计能量；
...
普通进食声是跨游戏参考音，50 ms RMS 固定为 `-14 dBFS`；BGM 的线性振幅是原基线的两倍（提高 100%），
单音窗口调整为比普通进食声低 6 dB，增强可听度且仍保持操作音优先。启动、金色食物、升级和失败
通过更长的旋律取得辨识度，而不是依赖更尖锐或更响的波形。构建会生成
...
数字分析能可靠防止数字响度和音色回归，但不能替代同一台设备上的 A/B 试听，也不能代表校准后的绝对声压。
