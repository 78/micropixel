...
void BatteryPeripheral::Initialize(i2c_master_bus_handle_t bus, i2c_master_dev_handle_t io_expander,
    ...
    const esp_err_t status = i2c_executor.Invoke(
        ...
        [](void* context) {
            static_cast<BatteryPeripheral*>(context)->InitializeOnWorker();
            return ESP_OK;
        },
        this);
    if (status != ESP_OK) {
...
void BatteryPeripheral::InitializeOnWorker() {
...
void BatteryPeripheral::NotifyExternalPowerChanged() {
...
bool BatteryPeripheral::ConfigurePowerDetectionInputs() {
...
bool BatteryPeripheral::ReadExternalPower(bool& connected) {
    if (io_expander_ == nullptr) {
        return false;
    }
    uint8_t port1 = 0U;
...
uint8_t BatteryPeripheral::Filter(uint8_t sample, int64_t now_us) {
    if (!filter_primed_) {
        filter_.fill(sample);
