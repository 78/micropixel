...
相机位于原点看向 +z，盒子前表面在深度 10 处正好填满 buffer，深度 9（`kBoxHalfDepth`）；宽高按 buffer 比例放缩，
大于 480 像素的面板以一半物理分辩率渲染。每帧的绘制记录：
...
- 每个球一条 `AdditiveSprite` 远墙光斑（球在远墙上的投影，越高越大越淡，贴墙时被球盖住而省略）、一条
  ...
  图集：11 个直径（14–52 px）的预光照球面，高光只向白色混合 55%，边缘覆盖率乘进亮度，在黑底上等价于抗锯齿；
