...
#include "platform/lvgl/ui/square_common/default_keyboard.hpp"
#include "platform/lvgl/ui/square_common/symbols.hpp"
...
void WifiSettingsUi::RenderLocked() {
    ...
    if (!scan_view_) {
        ...
        (void)CreateSystemLabel(wifi_text, "Wi-Fi", layout.heading_font, 0xf2f7ffU);
        const char* status = !model_.available  ? "Not available"
                             : !model_.enabled  ? "Off"
                             : model_.connected ? "Connected"
                                                : "Not connected";
        (void)CreateSystemLabel(wifi_text, status, layout.detail_font, 0x91a4bdU);
    ...
    if (!model_.enabled) {
        ...
        (void)CreateSystemLabel(hint, "Turn on Wi-Fi to view and connect to networks.", layout.body_font, 0x91a4bdU);
    } else if (scan_view_) {
        ...
        lv_obj_t* scan = CreateSystemActionButton(content, layout, "Connect to New Wi-Fi", 0x69a7ffU);
        lv_obj_add_event_cb(scan, OpenScanEvent, LV_EVENT_SHORT_CLICKED, this);
        (void)CreateSystemLabel(content, "SAVED NETWORKS", layout.detail_font, 0x91a4bdU);
        if (model_.saved_network_count == 0U) {
            lv_obj_t* empty = CreateSystemPanel(content, layout);
            (void)CreateSystemLabel(empty, "No saved networks", layout.body_font, 0x91a4bdU);
        }
...
void WifiSettingsUi::DrawNetworkRow(lv_obj_t* parent, const host_ui::WifiNetworkModel& network,
    ...
    (void)CreateWifiSignal(row, layout, network.rssi, network.connected ? 0xc5f36dU : 0x69a7ffU);
    lv_obj_t* text = CreateSystemColumn(row, 2);
    lv_obj_set_width(text, 0);
    lv_obj_set_flex_grow(text, 1);
    lv_obj_t* ssid = CreateSystemLabel(text, network.ssid.data(), layout.body_font, 0xf2f7ffU);
    ...
    (void)CreateSystemLabel(text, ConnectionText(network), layout.detail_font,
                            network.connected ? 0xc5f36dU : 0x91a4bdU);
    if (network.secured) {
        (void)CreateSystemLabel(row, kWifiLockSymbol, layout.body_font, 0x91a4bdU);
    }
    (void)CreateSystemLabel(row, LV_SYMBOL_RIGHT, layout.body_font, 0xf2f7ffU);
}
...
void WifiSettingsUi::DrawActionSheetLocked() {
    ...
    lv_obj_t* title = CreateSystemLabel(panel, network.ssid.data(), layout_->heading_font, 0xf2f7ffU);
    lv_obj_set_width(title, LV_PCT(100));
    lv_label_set_long_mode(title, LV_LABEL_LONG_DOT);
    lv_obj_t* connect = CreateSystemActionButton(panel, *layout_, network.connected ? "Disconnect" : "Connect",
                                                 network.connected ? 0xff8a91U : 0x69a7ffU);
    lv_obj_add_event_cb(connect, SheetConnectEvent, LV_EVENT_SHORT_CLICKED, this);
    lv_obj_t* forget = CreateSystemActionButton(panel, *layout_, "Forget Network", 0xff8a91U);
    lv_obj_add_event_cb(forget, SheetForgetEvent, LV_EVENT_SHORT_CLICKED, this);
    lv_obj_t* cancel = CreateSystemActionButton(panel, *layout_, "Cancel", 0x91a4bdU);
...
void WifiSettingsUi::DrawPasswordLocked() {
    ...
    (void)CreateSystemLabel(dialog, "Wi-Fi Password", layout_->heading_font, 0xf2f7ffU);
    lv_obj_t* ssid = CreateSystemLabel(dialog, password_network_.ssid.data(), layout_->detail_font, 0x91a4bdU);
    lv_obj_set_width(ssid, LV_PCT(100));
    lv_label_set_long_mode(ssid, LV_LABEL_LONG_DOT);

    const char* status = nullptr;
    uint32_t status_color = 0x91a4bdU;
    if (password_length_invalid_) {
        status = "Password must be at least 8 characters";
        status_color = 0xff737dU;
    } else {
        switch (password_connection_state_) {
            case host_ui::WifiConnectionState::kConnecting:
                status = "Connecting...";
                status_color = 0x69a7ffU;
                break;
    ...
    lv_obj_set_style_text_font(password_textarea_, BuiltinLatinFont(layout_->body_font), 0);
    lv_obj_t* actions = CreateSystemColumn(dialog, layout_->panel_gap);
    lv_obj_set_flex_flow(actions, LV_FLEX_FLOW_ROW);
    lv_obj_t* cancel = CreateSystemActionButton(actions, *layout_, "Cancel", 0x91a4bdU);
    lv_obj_set_width(cancel, 0);
    lv_obj_set_flex_grow(cancel, 1);
    lv_obj_add_event_cb(cancel, OverlayCancelEvent, LV_EVENT_SHORT_CLICKED, this);
    lv_obj_t* connect = CreateSystemActionButton(actions, *layout_, "Connect", 0x69a7ffU);
    ...
    keyboard_ = CreateDefaultKeyboard(overlay, password_textarea_, SystemFontRole::kSmall);
