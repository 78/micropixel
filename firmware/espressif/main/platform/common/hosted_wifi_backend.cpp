...
void HostedWifiBackend::HeapCapsDeleter::operator()(std::byte* value) const { heap_caps_free(value); }

void HostedWifiBackend::BindBackgroundExecutor(work::BackgroundExecutor& executor) { background_executor_ = &executor; }

std::expected<void, device::WifiError> HostedWifiBackend::Initialize() {
    if (initialized_) {
        return {};
    }
...
device::WifiSnapshot HostedWifiBackend::Snapshot() const {
...
void HostedWifiBackend::SetStateChangeSink(device::WifiStateChangeSink sink, void* context) {
    if (mutex_ == nullptr) {
...
std::expected<void, device::WifiError> HostedWifiBackend::SetEnabled(bool enabled) {
    {
        ScopedLock lock(mutex_);
        if (!initialized_) {
...
std::expected<void, device::WifiError> HostedWifiBackend::RequestScan() {
...
std::expected<void, device::WifiError> HostedWifiBackend::StartScan(ScanPurpose purpose) {
    uint8_t channel = 0U;
    {
        ScopedLock lock(mutex_);
        if (!initialized_ || !enabled_) {
...
void HostedWifiBackend::StartDiscovery() {
    {
        ScopedLock lock(mutex_);
...
void HostedWifiBackend::ScheduleDiscovery(bool advance_backoff) {
...
void HostedWifiBackend::ResetDiscoveryBackoff() {
    {
        ScopedLock lock(mutex_);
        discovery_backoff_index_ = 0U;
        discovery_connection_pending_ = false;
...
void HostedWifiBackend::DiscoveryTimerHandler(void* context) {
    auto* backend = static_cast<HostedWifiBackend*>(context);
    if (backend != nullptr) {
        backend->HandleDiscoveryTimer();
    }
}

void HostedWifiBackend::HandleDiscoveryTimer() { StartDiscovery(); }

std::expected<void, device::WifiError> HostedWifiBackend::ConnectSaved(std::string_view ssid) {
...
std::expected<void, device::WifiError> HostedWifiBackend::Connect(std::string_view ssid, std::string_view password) {
...
std::expected<void, device::WifiError> HostedWifiBackend::Disconnect() {
    {
        ScopedLock lock(mutex_);
        if (!initialized_ || !enabled_) {
            return std::unexpected(device::WifiError::kUnavailable);
...
std::expected<void, device::WifiError> HostedWifiBackend::Forget(std::string_view ssid) {
...
void HostedWifiBackend::WifiEventHandler(void* context, esp_event_base_t event_base, int32_t event_id,
                                         void* event_data) {
    auto* backend = static_cast<HostedWifiBackend*>(context);
    if (backend == nullptr) {
        return;
...
void HostedWifiBackend::HandleWifiEvent(int32_t event_id, void* event_data) {
...
void HostedWifiBackend::HandleGotIp() {
    {
        ScopedLock lock(mutex_);
        if (!associated_) {
            return;
...
void HostedWifiBackend::HandleScanDone() {
...
std::expected<void, device::WifiError> HostedWifiBackend::ConfigureAndConnect(const SavedProfile& profile,
...
int32_t HostedWifiBackend::FindSavedProfile(std::string_view ssid) const {
...
int32_t HostedWifiBackend::FindScanResult(std::string_view ssid) const {
...
bool HostedWifiBackend::ApplyScanTarget(std::string_view ssid, SavedProfile& profile) const {
...
bool HostedWifiBackend::BuildDiscoveryChannelsLocked() {
...
bool HostedWifiBackend::LoadSettings() {
    nvs_handle_t handle{};
...
bool HostedWifiBackend::SaveSettings() const {
...
void HostedWifiBackend::QueueSaveSettings() {
...
void HostedWifiBackend::SaveSettingsEntry(void* context) {
    auto* backend = static_cast<HostedWifiBackend*>(context);
    if (backend != nullptr) {
...
void HostedWifiBackend::ProcessPendingSettingsSaves() {
    for (;;) {
...
void HostedWifiBackend::RebuildSnapshotLocked() {
...
void HostedWifiBackend::UpdateScanResultsLocked(const void* raw_records, uint16_t count) {
...
void HostedWifiBackend::MergeDiscoveryScanResultsLocked(const void* raw_records, uint16_t count) {
    const auto* records = static_cast<const wifi_ap_record_t*>(raw_records);
