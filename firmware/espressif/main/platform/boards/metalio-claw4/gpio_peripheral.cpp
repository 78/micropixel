...
GpioPeripheral::GpioPeripheral() {
    for (uint32_t index = 0U; index < lines_.size(); ++index) {
        ...
        lines_[index].channel = kApplicationLines[index];
        lines_[index].line_number = kApplicationLines[index];
        lines_[index].pin = static_cast<gpio_num_t>(kApplicationLines[index]);
    }
...
esp_err_t GpioPeripheral::Initialize() {
    ESP_LOGI(kTag, "%u application GPIO lines ready; edge resources are lazy", static_cast<unsigned>(lines_.size()));
    return ESP_OK;
}

int32_t GpioPeripheral::GetInfo(device::PeripheralChannelId channel, micropixel_gpio_info_t& info_out) const {
...
int32_t GpioPeripheral::Open(device::PeripheralChannelId channel, uint16_t mode, uint16_t pull, uint16_t edge,
...
int32_t GpioPeripheral::ConfigureInput(Line& line, uint16_t pull, uint16_t edge,
                                       device::GpioPeripheralEdgeSink edge_sink, void* edge_context) {
...
int32_t GpioPeripheral::ConfigureOutput(Line& line, bool initial_value) {
    gpio_config_t config{};
    config.pin_bit_mask = 1ULL << static_cast<uint32_t>(line.pin);
...
int32_t GpioPeripheral::ConfigurePwm(Line& line, uint32_t frequency_hz, uint16_t initial_duty_per_mille) {
...
int32_t GpioPeripheral::Read(device::PeripheralChannelId channel, bool& value_out) const {
    const Line* line = Find(channel);
...
int32_t GpioPeripheral::Write(device::PeripheralChannelId channel, bool value) {
    Line* line = Find(channel);
...
int32_t GpioPeripheral::SetPwmDuty(device::PeripheralChannelId channel, uint16_t duty_per_mille) {
    Line* line = Find(channel);
...
void GpioPeripheral::Close(device::PeripheralChannelId channel) {
    Line* line = Find(channel);
    if (line == nullptr || !line->active) {
        return;
    }
...
void GpioPeripheral::SuspendEvents() {
...
int32_t GpioPeripheral::ResumeEvents() {
    if (!events_suspended_) {
        return MICROPIXEL_STATUS_OK;
    }
    if (edge_line_count_ == 0U) {
...
GpioPeripheral::Line* GpioPeripheral::Find(device::PeripheralChannelId channel) {
    for (Line& line : lines_) {
...
const GpioPeripheral::Line* GpioPeripheral::Find(device::PeripheralChannelId channel) const {
...
bool GpioPeripheral::StartEdgeWorker() {
    if (worker_ != nullptr) {
        return true;
    }
...
void GpioPeripheral::StopEdgeWorker() {
    if (worker_ != nullptr) {
...
void GpioPeripheral::ReleaseEdgeResources() {
...
void IRAM_ATTR GpioPeripheral::OnEdge(void* context) {
    auto& line = *static_cast<Line*>(context);
    GpioPeripheral& peripheral = *line.owner;
...
void GpioPeripheral::WorkerEntry(void* context) {
    static_cast<GpioPeripheral*>(context)->Worker();
    vTaskDelete(nullptr);
}

void GpioPeripheral::Worker() {
