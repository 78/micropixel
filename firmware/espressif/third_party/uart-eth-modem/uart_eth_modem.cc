...
UartEthModem::UartEthModem(const Config& config) : config_(config) {
    ...
    if (!event_group_) {
        ...
        // Start reports allocation failure to the board owner.
    }

    for (size_t i = 0; i < tx_done_.size(); ++i) {
        tx_done_[i] = xSemaphoreCreateBinaryStatic(&tx_done_storage_[i]);
    }
...
UartEthModem::~UartEthModem() {
    // The owning board must stop successfully before destroying this object.
    // A timed-out worker still owns its memory; never free it underneath the task.
...
esp_err_t UartEthModem::Start(bool flight_mode) {
    ...
    if (!GPIO_IS_VALID_OUTPUT_GPIO(config_.tx_pin) || !GPIO_IS_VALID_OUTPUT_GPIO(config_.mrdy_pin) ||
        !GPIO_IS_VALID_GPIO(config_.rx_pin) || !GPIO_IS_VALID_GPIO(config_.srdy_pin) ||
        config_.uart_num < UART_NUM_0 || config_.uart_num >= UART_NUM_MAX || config_.baud_rate <= 0) {
        return ESP_ERR_INVALID_ARG;
    }
    xEventGroupClearBits(event_group_, 0x00ffffff);
    main_task_ = nullptr;
    init_task_ = nullptr;
    tx_task_ = nullptr;
    isr_barrier_pending_ = false;
    initialized_ = false;
    seq_no_ = 0;
    network_event_ = UartEthModemEvent::Disconnected;
    ...
    if (!main_task_ || !init_task_ || !tx_task_) {
        ...
        // Tasks that were never created cannot report completion. Join every
        // task that did start before releasing any queue, UART or DMA storage.
        uint32_t absent = 0;
        if (!main_task_) absent |= kEventMainTaskDone;
        if (!init_task_) absent |= kEventInitTaskDone;
        if (!tx_task_) absent |= kEventTxTaskDone;
        xEventGroupSetBits(event_group_, absent);
...
esp_err_t UartEthModem::FailStart(esp_err_t error) {
    // Before task creation, stop interrupts and DMA before deleting their queues.
    stop_flag_ = true;
    initializing_ = false;
...
void UartEthModem::RequestStop() {
    // Serialize with producers, so no new frame can enter after shutdown begins.
    std::lock_guard<std::mutex> lock(tx_mutex_);
    stop_flag_ = true;
    initializing_ = false;
    if (event_group_) xEventGroupSetBits(event_group_, kEventStop);
    if (event_queue_) {
        Event event = {.type = EventType::Stop, .rx_buffer = nullptr};
        xQueueSend(event_queue_, &event, 0);
    }
    if (tx_queue_) {
        TxFrame* wake = nullptr;
        xQueueSend(tx_queue_, &wake, 0);
    }
}

esp_err_t UartEthModem::Stop() {
    if (!event_queue_) return ESP_OK;
    const auto caller = xTaskGetCurrentTaskHandle();
    RequestStop();
    if (caller == main_task_ || caller == init_task_ || caller == tx_task_) {
        // Event callbacks may request a stop, but the board owner must join it.
        return ESP_ERR_INVALID_STATE;
    }
    const auto bits = xEventGroupWaitBits(event_group_, kEventAllTasksDone,
                                          pdFALSE, pdTRUE, pdMS_TO_TICKS(10000));
    if ((bits & kEventAllTasksDone) != kEventAllTasksDone) {
        ESP_LOGE(kTag, "Tasks still stopping; resources retained");
        return ESP_ERR_TIMEOUT;
    }
    // SetBitsFromISR is deferred through the timer task. Remove the producer,
    // then place a barrier behind its queued callbacks before freeing storage.
    DeinitGpio();
    if (!isr_barrier_pending_) {
        xEventGroupClearBits(event_group_, kEventIsrDrained);
        if (xTimerPendFunctionCall([](void* group, uint32_t) {
                xEventGroupSetBits(static_cast<EventGroupHandle_t>(group), kEventIsrDrained);
            }, event_group_, 0, pdMS_TO_TICKS(100)) != pdPASS) return ESP_ERR_TIMEOUT;
        isr_barrier_pending_ = true;
    }
    if (!(xEventGroupWaitBits(event_group_, kEventIsrDrained, pdFALSE, pdTRUE,
                              pdMS_TO_TICKS(1000)) & kEventIsrDrained)) return ESP_ERR_TIMEOUT;
    // No modem task remains. External AT calls wake on the persistent stop bit.
    std::lock_guard<std::mutex> at_lock(at_mutex_);
    ...
    initialized_ = false;
    handshake_done_ = false;
    SetNetworkEvent(UartEthModemEvent::Disconnected);
    return ESP_OK;
}

esp_err_t UartEthModem::SendAt(const std::string& cmd, std::string& response, uint32_t timeout_ms) {
    return SendAtCommon(cmd, response, timeout_ms, {});
}

esp_err_t UartEthModem::SendAtCollectUntil(const std::string& cmd, std::string& response,
                                         uint32_t timeout_ms, const char* done_marker) {
    if (!done_marker || !done_marker[0]) return ESP_ERR_INVALID_ARG;
    return SendAtCommon(cmd, response, timeout_ms, done_marker);
}

esp_err_t UartEthModem::SendAtCommon(const std::string& cmd, std::string& response,
    ...
    if (result == ESP_OK) {
        ...
        if (bits & kEventStop) result = ESP_ERR_INVALID_STATE;
        else if (!(bits & kEventAtResponse)) result = ESP_ERR_TIMEOUT;
    }
    std::lock_guard<std::mutex> lock(response_mutex_);
...
int UartEthModem::RegistrationStatus() {
    std::lock_guard<std::mutex> lock(info_mutex_);
    return cell_info_.stat;
}
...
esp_err_t UartEthModem::InitGpio() {
    ...
    // Start enables interrupts only after every worker has been created.

    return ESP_OK;
}

esp_err_t UartEthModem::InitIotEth() {
    std::unique_lock<std::mutex> lock(netif_mutex_);
    netif_ready_ = false;
    link_up_ = false;
    ...
    ret = esp_netif_attach(eth_netif_, glue_);
    if (ret != ESP_OK) {
        lock.unlock();
        DeinitIotEth();
        return ret;
...
void UartEthModem::DeinitIotEth() {
    ...
    netif_ready_ = false;
    link_up_ = false;
    // Do not hold the mutex while unregister waits for an in-flight IP handler.
    lock.unlock();
...
void UartEthModem::TxTaskRun() {
    ...
    while (!stop_flag_.load()) {
        ...
        if (xQueueReceive(tx_queue_, &frame, portMAX_DELAY) == pdTRUE) {
            if (stop_flag_.load()) {
                CompleteTx(frame, ESP_ERR_INVALID_STATE);
                break;
            }
            ...
            ret = uart_uhci_.Transmit(frame->data.data(), frame->length);
            if (ret != ESP_OK) {
            ...
            // Wait for ACK (SRDY high)
            {
                EventBits_t bits = xEventGroupWaitBits(
                    ...
                    pdFALSE,  // keep stop visible to every task
...
void UartEthModem::MainTaskRun() {
    ...
    while (!stop_flag_.load()) {
        for (auto& dropped : dropped_rx_) {
            if (auto* buffer = dropped.exchange(nullptr, std::memory_order_acquire)) {
                reassembly_size_ = 0;
                reassembly_expected_ = 0;
...
bool IRAM_ATTR UartEthModem::UhciRxCallbackStatic(const UartUhci::RxEventData& data, void* user_data) {
    ...
    if (xQueueSendFromISR(self->event_queue_, &event, &xHigherPriorityTaskWoken) != pdTRUE) {
        self->dropped_rx_[data.buffer->index].store(data.buffer, std::memory_order_release);
    }
...
void UartEthModem::InitTaskRun() {
    ...
    xEventGroupWaitBits(event_group_, kEventStart | kEventStop, pdFALSE, pdFALSE, portMAX_DELAY);
    ...
    // Run initialization sequence based on mode
    {
        ...
        if (!flight_mode_ && handshake_done_ && !stop_flag_) {
...
// Every queued pointer refers to fixed pool storage, including completion state.
...
esp_err_t UartEthModem::EnqueueTxFrame(const uint8_t* data, size_t length) {
    return SubmitFrame(data, length, FrameType::kEthernet, false);
}

esp_err_t UartEthModem::SendFrame(const uint8_t* data, size_t length, FrameType type) {
    return SubmitFrame(data, length, type, true);
}

esp_err_t UartEthModem::SubmitFrame(const uint8_t* data, size_t length, FrameType type, bool wait) {
    if (!data || length == 0 || length > kMaxFrameSize - sizeof(FrameHeader)) return ESP_ERR_INVALID_SIZE;
    TxFrame* frame;
...
void UartEthModem::HandleEthFrame(uint8_t* data, size_t length) {
    ...
    // Forward to iot_eth stack only after its input path is fully attached.
    std::lock_guard<std::mutex> lock(netif_mutex_);
    if (netif_ready_ && mediator_) {
...
void UartEthModem::ParseAtResponse(std::string_view response) {
    std::unique_lock<std::mutex> lock(info_mutex_);
    if (debug_enabled_.load()) {
        ESP_LOGI(kTag, "AT<<< %.*s", static_cast<int>(response.size()), response.data());
    }
    ...
    if (cereg_pos != std::string::npos) {
        char line[128]{};
        const auto tail = response.substr(cereg_pos);
        memcpy(line, tail.data(), std::min(tail.size(), sizeof(line) - 1));
        int n = 0, stat = 0;
        char tac[16] = {0}, ci[16] = {0};
        int act = 0;

        // Try format 1: +CEREG: n,stat,"tac","ci",act (with n parameter)
        if (sscanf(line, "+CEREG: %d,%d,\"%15[^\"]\",\"%15[^\"]\",%d", &n, &stat, tac, ci, &act) == 5) {
        ...
        else if (sscanf(line, "+CEREG: %d,\"%15[^\"]\",\"%15[^\"]\",%d", &stat, tac, ci, &act) == 4) {
            cell_info_.stat = stat;
            cell_info_.tac = tac;
            cell_info_.ci = ci;
            cell_info_.act = act;
        ...
        else if (sscanf(line, "+CEREG: %d,%d", &n, &stat) == 2) {
            cell_info_.stat = stat;
        }
        ...
        const int registration = cell_info_.stat;
        lock.unlock();
        if (registration == 2) {
            NotifyLink(false);
        ...
        } else if (registration == 1 || registration == 5) {
            if (handshake_done_) NotifyLink(true);
        } else {
            NotifyLink(false);
            SetNetworkEvent(UartEthModemEvent::Disconnected);
        }
    } else if (response.find("+ECNETDEVCTL: 0") != std::string_view::npos) {
        lock.unlock();
        NotifyLink(false);
        SetNetworkEvent(UartEthModemEvent::Disconnected);
    }
...
void UartEthModem::NotifyLink(bool up) {
    std::lock_guard<std::mutex> lock(netif_mutex_);
    if (!up) xEventGroupClearBits(event_group_, kEventNetworkReady);
    if (netif_ready_ && mediator_ && link_up_ != up) {
        link_up_ = up;
        iot_eth_link_t link = up ? IOT_ETH_LINK_UP : IOT_ETH_LINK_DOWN;
        mediator_->on_stage_changed(mediator_, IOT_ETH_STAGE_LINK, &link);
    }
}
...
bool UartEthModem::WaitForRegistration(uint32_t timeout_ms) {
    ...
    while (!stop_flag_.load()) {
        if (SendAt("AT+CEREG?", resp, 1000) == ESP_OK) {
            ParseAtResponse(resp);
            if (const int status = RegistrationStatus(); status == 1 || status == 5) {
                return true;
            }
            if (RegistrationStatus() == 3) {
...
void UartEthModem::IpEventHandler(void* arg, esp_event_base_t event_base,
    ...
    if (event_base != IP_EVENT ||
        (event_id != IP_EVENT_ETH_GOT_IP && event_id != IP_EVENT_ETH_LOST_IP)) return;
    auto* acquired = static_cast<ip_event_got_ip_t*>(event_data);
    {
        std::lock_guard<std::mutex> lock(self->netif_mutex_);
        if (!acquired || acquired->esp_netif != self->eth_netif_ || !self->netif_ready_ || self->stop_flag_) return;
    }
    if (event_id == IP_EVENT_ETH_GOT_IP) {
        self->SetNetworkEvent(UartEthModemEvent::Connected);
        xEventGroupSetBits(self->event_group_, kEventNetworkReady);
    } else {
        xEventGroupClearBits(self->event_group_, kEventNetworkReady);
        self->SetNetworkEvent(UartEthModemEvent::Disconnected);
    }
}
...
void UartEthModem::CleanupResources(bool cleanup_iot_eth) {
    ...
    for (auto& dropped : dropped_rx_) dropped.store(nullptr);
