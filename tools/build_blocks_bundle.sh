...
app_dir="$workspace_root/guest/apps/blocks"
app_build_dir="${P4_BLOCKS_BUILD_DIR:-$workspace_root/build/apps/blocks}"
guest_dir="${P4_GUEST_OUTPUT_DIR:-$app_build_dir}"
asset_dir="${P4_BLOCKS_ASSET_DIR:-$app_build_dir/assets}"
bundle_output="${P4_BUNDLE_OUTPUT:-$app_build_dir/blocks.bundle.bin}"
...
sfx_header="$asset_dir/blocks_sfx_profiles.hpp"
...
if not isinstance(sources, list) or not sources or not all(isinstance(source, str) for source in sources):
    raise SystemExit("blocks app manifest requires a non-empty string sources list")
if sources[0] != manifest.get("source"):
    raise SystemExit("blocks app manifest source must be the first sources entry")
...
python3 "$workspace_root/tools/build_app_bundle.py" \
    ...
    --emit-cpp-header "$asset_dir/blocks_assets.hpp" \
    --cpp-namespace blocks_assets
...
P4_GUEST_OUTPUT_DIR="$guest_dir" \
    ...
    "${guest_sources[0]}" blocks "${guest_sources[@]:1}"

python3 "$workspace_root/tools/build_app_bundle.py" \
    --aot "$guest_dir/blocks.aot" \
    --app-manifest "$app_dir/app.json" \
    --resource-pack "$resource_pack" \
    --output "$bundle_output"

echo "Blocks app built: $bundle_output"
ls -lh "$bundle_output"
