...
void SensorPeripheral::Initialize(i2c_master_bus_handle_t bus, buses::I2cExecutor& i2c_executor) {
    ...
    if (acceleration_.available() && create_timer(acceleration_sampler_, kAcceleration, "micropixel_accel") != ESP_OK) {
        ESP_LOGW(kTag, "acceleration timer unavailable");
    }
    if (magnetic_field_.available() &&
        create_timer(magnetic_field_sampler_, kMagneticField, "micropixel_magnet") != ESP_OK) {
...
esp_err_t SensorPeripheral::SampleOnWorker(void* context) {
    ...
    if (sampler->channel == kAcceleration) {
        status = sampler->owner->ReadAcceleration(values);
    } else if (sampler->channel == kMagneticField) {
...
int32_t SensorPeripheral::ReadAcceleration(device::SensorValues& values_out) {
    float values[3]{};
...
int32_t SensorPeripheral::ReadMagneticField(device::SensorValues& values_out) {
    float values[3]{};
