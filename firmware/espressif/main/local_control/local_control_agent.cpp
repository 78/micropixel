...
LocalControlAgent::LocalControlAgent(device::LocalControlBackend& backend,
    ...
    if (response_queue_bytes_ != nullptr && snapshot_workspace_ != nullptr) {
        ...
        ESP_LOGI(kTag, "local control workspaces allocated in PSRAM: queue=%zu snapshot=%zu bytes",
...
bool LocalControlAgent::ReceiveHostResult(void* context, const remote_control::RemoteControlHostResult& result) {
...
void LocalControlAgent::InstallTimeoutElapsed(void* context) {
    ...
    agent->backend_.NotifyResponseReady();
}
...
bool LocalControlAgent::HandleHostResult(const remote_control::RemoteControlHostResult& result) {
    uint32_t request_id = 0U;
    const std::string_view command_id(result.command_id.data());
    // Keep the established internal route key stable. It is not part of the
    // serial transport protocol, but RemoteControlAgent uses it to isolate
    // locally-originated Host commands from network commands.
...
bool LocalControlAgent::QueueHostCommand(uint32_t request_id, remote_control::RemoteControlHostCommandType type,
