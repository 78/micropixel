...
def generate_header(
    ...
    for locale, catalog in catalogs.items():
        lines.append(f"inline constexpr const char* const {symbol_name(locale)}[kStringCount]{{")
    ...
    lines += [
        "class Catalog final {",
        "   public:",
        "    [[nodiscard]] const char* Get(Id id) const {",
        "        const size_t index = static_cast<size_t>(id);",
        "        return strings_ != nullptr && index < kStringCount ? strings_[index] : \"\";",
        "    }",
        "",
        "   private:",
        "    explicit constexpr Catalog(const char* const* strings) : strings_(strings) {}",
    ...
    for locale in catalogs:
        lines.append(
            f"    if (detail::Equal(tag, {cpp_string(locale)})) return Catalog{{{symbol_name(locale)}}};"
        )
    for locale in catalogs:
        lines.append(
            f"    if (detail::SameLanguageAndScript(tag, {cpp_string(locale)})) "
            f"return Catalog{{{symbol_name(locale)}}};"
        )
    for locale in catalogs:
        lines.append(
    ...
    lines += [f"    return Catalog{{{symbol_name(source.default_locale)}}};", "}"]
    if source.guest_api:
        lines += [
            "",
