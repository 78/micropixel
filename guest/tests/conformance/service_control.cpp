...
int main() {
    ...
    if (micropixel_service_open(
        ...
        (service.flags & MICROPIXEL_SERVICE_FLAG_SUBMIT) == 0U || service.service_handle == 0U) {
        return 122;
    }
    ...
    if (micropixel_service_call(service.service_handle, MICROPIXEL_GRAPHICS_METHOD_GET_INFO, &dummy, 1U,
                                reinterpret_cast<uint8_t*>(&renderer), sizeof(renderer),
                                &response_size) != MICROPIXEL_STATUS_INVALID_ARGUMENT) {
        return 123;
    }
    if (micropixel_service_call(service.service_handle, 0xffffffffU, &dummy, 0U, reinterpret_cast<uint8_t*>(&renderer),
                                sizeof(renderer), &response_size) != MICROPIXEL_STATUS_UNSUPPORTED) {
        return 124;
    }
    if (micropixel_service_call(service.service_handle, MICROPIXEL_GRAPHICS_METHOD_GET_INFO, &dummy, 0U,
                                reinterpret_cast<uint8_t*>(&renderer), sizeof(renderer),
    ...
    if (micropixel_service_open(
        ...
        random_service.service_id != MICROPIXEL_SERVICE_RANDOM || random_service.service_handle == 0U ||
    ...
    if (micropixel_service_call(random_service.service_handle, MICROPIXEL_RANDOM_METHOD_GET_U32, &dummy, 1U,
                                reinterpret_cast<uint8_t*>(&random_value), sizeof(random_value),
                                &response_size) != MICROPIXEL_STATUS_INVALID_ARGUMENT) {
        return 128;
    }
    if (micropixel_service_call(random_service.service_handle, MICROPIXEL_RANDOM_METHOD_GET_U32, nullptr, 0U,
                                reinterpret_cast<uint8_t*>(&random_value), sizeof(random_value),
                                &response_size) != MICROPIXEL_STATUS_OK ||
    ...
    constexpr ServiceExpectation kHardwareServices[]{
        ...
        {MICROPIXEL_SERVICE_HAPTICS, MICROPIXEL_HAPTICS_INTERFACE_MAJOR, MICROPIXEL_HAPTICS_INTERFACE_MINOR,
         MICROPIXEL_SERVICE_FLAG_CALL | MICROPIXEL_SERVICE_FLAG_EVENTS},
        {MICROPIXEL_SERVICE_POWER, MICROPIXEL_POWER_INTERFACE_MAJOR, MICROPIXEL_POWER_INTERFACE_MINOR,
         MICROPIXEL_SERVICE_FLAG_CALL},
    };
    ...
    for (const ServiceExpectation& expected : kHardwareServices) {
        ...
        if (micropixel_service_open(expected.id, MICROPIXEL_INTERFACE_VERSION(expected.major, expected.minor),
            ...
            (hardware_service.flags & expected.flags) != expected.flags || hardware_service.service_handle == 0U) {
            return 130;
        }
        if (expected.id == MICROPIXEL_SERVICE_DEVICES) {
            devices_handle = hardware_service.service_handle;
        }
    }
    ...
    if (micropixel_service_call(devices_handle, MICROPIXEL_DEVICES_METHOD_LIST,
        ...
        device_list.count > MICROPIXEL_DEVICES_LIST_PAGE_SIZE || device_list.count > device_list.total_count) {
        return 131;
    }

    micropixel_service_info_t timers{};
    if (micropixel_service_open(MICROPIXEL_SERVICE_TIMER, MICROPIXEL_INTERFACE_VERSION(1U, 0U), &timers,
                                sizeof(timers)) != MICROPIXEL_STATUS_OK)
        return 132;
    struct ExtendedHandle {
        micropixel_handle_request_t request;
        uint32_t extra;
    } bad{{sizeof(micropixel_handle_request_t), 1U, 1U}, 0U};
    auto cancel = [&](uint32_t length) {
        return micropixel_service_call(timers.service_handle, MICROPIXEL_TIMER_METHOD_CANCEL,
                                       reinterpret_cast<const uint8_t*>(&bad), length, nullptr, 0U, &response_size);
    };
    if (cancel(sizeof(bad.request)) != MICROPIXEL_STATUS_INVALID_ARGUMENT) return 133;
    bad.request.reserved0 = 0U;
    if (cancel(sizeof(bad)) != MICROPIXEL_STATUS_INVALID_ARGUMENT) return 134;
    bad.request.size = sizeof(bad);
    if (cancel(sizeof(bad)) != MICROPIXEL_STATUS_INVALID_ARGUMENT) return 135;
