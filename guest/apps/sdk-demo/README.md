...
构建阶段只生成
`build/apps/sdk-demo/generated/demo_assets.hpp`，页面从这个头文件取得 `AssetId` 和帧布局。Demo 面向
...
`Playing`。离开页面会调用 `StopAll()`。目录与产物名为 `sdk-demo`；App ID 保留 `micropixel.demo`，增量安装仍更新原应用。
Storage 页使用该 App ID 的私有 KV namespace，
计数值会跨应用和设备重启保留。
