...
micropixel CLI
  ...
  -> DevelopmentLocalControlTransport / device::LocalControl
  -> LocalControlAgent
  -> ControlDispatcher 的共享有界 Host 命令队列
  -> HostController
  -> AppStore / BundleFS
```

Platform 只拥有 USB 字节流和开发截图/触摸命令。`LocalControlAgent` 解析本地控制协议，但不直接
...
Remote Control 不会关闭日志采集，也不会改变 USB 游标和保留策略。最近一次 App 失败同样保存在共享 Host
snapshot 中，`APP_LAST_ERROR` 不依赖网络连接或云端 Job。
...
联网发布通道获取签名固件，因此要求设备网络和 Remote Control 服务配置可用。
...
截图复制最终送屏缓冲，由 SoC JPEG 外设编码为 JPEG，再按“ASCII header + 精确长度二进制 payload +
sequence end marker”传输。该路径反映用户实际看到的合成结果，并避免软件 PNG 编码占用 CPU。
...
`micropixel --transport usb run` 在客户端显式编排 package、幂等 `APP_STOP`、事务安装、`APP_START` 和
