...
std::expected<AppRuntime, AppRuntimeError> AppRuntime::Initialize(device::DeviceServices& devices,
    ...
    ESP_LOGI(kTag,
             "Guest elastic memory: linear-ceiling=%d pages, PSRAM-reserve=%d bytes, max child threads=%d, "
             "watchdog=%d ms",
             CONFIG_MICROPIXEL_GUEST_LINEAR_MEMORY_MAX_PAGES, CONFIG_MICROPIXEL_GUEST_PSRAM_RESERVE_BYTES,
             CONFIG_WAMR_RUNTIME_MAX_GUEST_THREADS, CONFIG_WAMR_DEFAULT_WATCHDOG_TIMEOUT_MS);
