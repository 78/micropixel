...
std::expected<host_ui::ScreenCapture, host_ui::SystemUiError> MosaicoPresentation::CaptureScreenJpeg() {
    ...
    if (state_.guest_graphics.DirectlyScannedAppSurface(surface) &&
        surface.format == graphics::SurfacePixelFormat::kRgb565) {
        return lvgl::CaptureScreenJpeg(state_.display, kPanelWidth, kPanelHeight,
                                       {.pixels = surface.pixels,
                                        .stride = surface.stride,
                                        .format = lvgl::DisplayCapturePixelFormat::kRgb565,
                                        .ready = &kReady});
    }
    if (state_.guest_graphics.DirectSurfaceActive()) {
        // The presenter copies the front buffer into a temporary panel-sized
        // frame while it still owns it; the copy is encoded and freed here.
        const size_t stride = static_cast<size_t>(kPanelWidth) * 2U;
        auto* frame =
            static_cast<uint8_t*>(heap_caps_malloc(stride * kPanelHeight, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
        if (frame != nullptr) {
            std::expected<host_ui::ScreenCapture, host_ui::SystemUiError> result =
                std::unexpected(host_ui::SystemUiError::kUnavailable);
            if (state_.guest_graphics.CaptureDirectSurfaceFront(frame, static_cast<uint32_t>(stride), kPanelWidth,
                                                                kPanelHeight)) {
                result = lvgl::CaptureScreenJpeg(state_.display, kPanelWidth, kPanelHeight,
                                                 {.pixels = frame,
