...
纯黑背景、线框盒体、逐球光晕和远墙光斑面向 AMOLED 的黑色设计。不依赖其他 App 或外部物理库，运行画面不依赖图片素材。

大厅封面使用 `assets/launch.jpg`（720×720 JPEG），通过 `assets/manifest.json` 打入 Bundle。
封面由 ImageGen 生成，本地 PNG 原图不纳入 Git。

## 运行要求

需要 MicroPixel 固件 **0.8.0 或更新版本**。商店当前不支持按固件版本自动拦截安装，安装前请确认固件版本。
...
大于 480 像素的面板以一半物理分辨率渲染（Claw4 720×720 → 360 buffer，Host 2× 放大），
降低球群靠近玻璃时的像素填充与内存带宽需求。每帧的绘制记录：
