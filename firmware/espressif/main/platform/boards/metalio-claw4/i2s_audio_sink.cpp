...
esp_err_t InitializeBtAudioMode() {
    ...
    ESP_RETURN_ON_ERROR(uart_driver_install(board::kAudioControlUart, 512, 512, 0, nullptr, 0), kTag,
                        "install audio UART");
    esp_err_t status = uart_param_config(board::kAudioControlUart, &config);
    if (status == ESP_OK) {
        status = uart_set_pin(board::kAudioControlUart, board::kAudioControlTx, board::kAudioControlRx,
                              UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    }
    if (status != ESP_OK) {
        (void)uart_driver_delete(board::kAudioControlUart);
        return status;
    }
    vTaskDelay(pdMS_TO_TICKS(1000));
    constexpr char kReceivePath[] = "AT+RX=2\r\n";
    constexpr char kModeOne[] = "AT+MODE=1\r\n";
    if (uart_write_bytes(board::kAudioControlUart, kReceivePath, sizeof(kReceivePath) - 1U) < 0) {
        return ESP_FAIL;
    }
    (void)uart_wait_tx_done(board::kAudioControlUart, pdMS_TO_TICKS(500));
    vTaskDelay(pdMS_TO_TICKS(700));
    if (uart_write_bytes(board::kAudioControlUart, kModeOne, sizeof(kModeOne) - 1U) < 0) {
        return ESP_FAIL;
    }
    (void)uart_wait_tx_done(board::kAudioControlUart, pdMS_TO_TICKS(500));
    vTaskDelay(pdMS_TO_TICKS(700));
    ...
    const int response_size =
        uart_read_bytes(board::kAudioControlUart, response, sizeof(response) - 1U, pdMS_TO_TICKS(200));
    if (response_size > 0) {
...
esp_err_t I2sAudioSink::Initialize() {
    ...
    i2s_chan_config_t channel_config = I2S_CHANNEL_DEFAULT_CONFIG(board::kAudioI2sPort, I2S_ROLE_SLAVE);
