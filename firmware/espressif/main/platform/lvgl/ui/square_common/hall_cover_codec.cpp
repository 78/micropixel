...
bool DecodeHallCoverRgb888(const host_ui::HallCoverModel& source, uint32_t target_size, uint32_t corner_radius,
    ...
    if (source.format == host_ui::HallCoverFormat::kJpeg) {
        decoded = DecodeJpegCoverRgb888(source.data, source.size, source.width, source.height, destination, target_size,
                                        target_size, destination_stride);
    } else if (source.format == host_ui::HallCoverFormat::kPng) {
        decoded = DecodePngCoverRgb888(source.data, source.size, source.width, source.height, destination, target_size,
                                       target_size, destination_stride, top_background_rgb);
