...
- `FirmwareApp` 是唯一组合根，负责平台、设备服务和 Runtime 的构造顺序；
...
app_main (ESP-IDF C entry)
    └── FirmwareApp (composition root)
        ...
        │   └── GuestContext / EventQueue / TouchEventBridge
...
| WAMR 全局初始化 | `WamrRuntime` | move-only RAII |
| AOT package buffer/mapping | `AotPackage` | move-only RAII |
| module、instance、exec env | Runtime session | 分离的 move-only RAII |
| Guest session 编排 | `GuestContext` | 按值组合 session services |
...
- CMake 为每个配置只选择一组 Metalio-Claw4 或 Null Platform 源码；`ConfiguredPlatform()` 返回持有板级状态的
  单一对象，`FirmwareApp` 再把四个 Device backend 显式注入 `DeviceServices` 和 `Engine`；
...
- `ResourceService` 只负责任务调度，JPEG/PNG 生命周期以及 bitmap/offscreen-surface handle、可变性和
  PSRAM 配额分别归 `BitmapDecoder`、`BitmapStore`；surface update frame 由 Graphics backend 合并每个
