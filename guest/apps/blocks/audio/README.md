...
`sfx.json` 是 Blocks 音效的唯一参数源。构建流程先按当前 Host 的真实规则合成 16 kHz PCM，
...
快速落地是游戏内参考音。为提升 Juicy Blocks 在目标设备上的整体可听度，所有 Tone 的线性振幅在原基线
上统一提高 50%（约 `+3.52 dB`），其 50 ms RMS 目标相应设为 `-10.5 dBFS`。这是有意高于跨游戏
`-14 dBFS` 基线的例外；移动/软降仍比快速落地低 14 dB，自然落地仍低 8 dB。启动、消行和 Game Over
依靠时长与旋律取得辨识度，不靠超高瞬时电平。评分用于稳定地权衡和防止回归，最终舒适度仍以同一台
设备上的 A/B 试听为准。
