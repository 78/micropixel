...
在资源打开时校验哈希，AOT 仍独立复制。安装校验继续以 4 KiB 块流式计算哈希。
...
App Store 升级保留旧包，直到新包写入、校验并提交 Catalog。下载前，Remote Control 通过
ControlDispatcher 的固定安装状态请求 Host supervisor 预检查；仅 supervisor 访问 AppStore，按当前
目标介质的空闲字节数和 data block 大小计算“新包大小 + 一个预留块”。相同已安装 digest 无需新增空间。
预检查有界等待，失败时不分配包缓冲区、不发起包下载，并通过安装结果和大厅弹窗报告错误；空间不足
提示需释放的 KiB 数。请求进入安装流程后，请求状态恢复空闲；安装失败仅保留在错误弹窗中，不再
作为升级菜单的请求失败提示。关闭弹窗或开始新的安装会清除旧错误。预检查不预留块，写入前仍独立校验实际
容量；任何提交前失败保留旧包，不能为腾空间自动卸载旧版本。安装过程的 RAM、已安装包和暂存包
元数据读取复用一个 RAII 管理的 PSRAM 工作区，避免多份元数据及返回值临时对象叠加在 Host 栈上；
工作区分配失败发生在 Flash 写入之前。
