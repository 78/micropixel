...
esp_err_t Gt911Input::Initialize(i2c_master_bus_handle_t i2c_bus, common::I2cExecutor& i2c_executor) {
...
void IRAM_ATTR Gt911Input::InterruptEntry(esp_lcd_touch_handle_t) {
    ...
    if (!instance->i2c_executor_->PostFromIsr(common::I2cExecutor::Priority::kHigh, ProcessEntry, instance,
...
void Gt911Input::ProcessInterrupt() {
    ...
    if (interrupts_.load(std::memory_order_acquire) != observed_interrupts &&
        ...
        !i2c_executor_->Post(common::I2cExecutor::Priority::kHigh, ProcessEntry, this)) {
        work_pending_.store(false, std::memory_order_release);
    }
}
