...
生成的 Wasm/AOT 位于 `build/guest-p4/`，不进入版本控制。`event_wait` 和 `touch_pressure` 在真机运行时
需要 Host 的合成事件钩子；使用 `firmware/espressif/sdkconfig.p4-conformance.defaults` 构建专用固件。
