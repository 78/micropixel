...
esp_err_t Tca9555PowerKey::Initialize(i2c_master_dev_handle_t io_expander, buses::I2cExecutor& i2c_executor) {
    ...
    interrupt_config.pin_bit_mask = 1ULL << board::kIoExpanderInterrupt;
    ...
    const int initial_interrupt_level = gpio_get_level(board::kIoExpanderInterrupt);
...
esp_err_t Tca9555PowerKey::PrepareForLightSleep() {
    ...
    for (uint32_t attempt = 0U; attempt < kWakeLineDrainAttempts; ++attempt) {
        ...
        if (gpio_get_level(board::kIoExpanderInterrupt) != 0) {
...
esp_err_t Tca9555PowerKey::ConfigureInterrupt() {
    esp_err_t status = gpio_set_intr_type(board::kIoExpanderInterrupt, GPIO_INTR_LOW_LEVEL);
    if (status != ESP_OK) {
        return status;
    }
...
uint8_t Tca9555PowerKey::ReadKeyLevel() {
    uint8_t port0 = 0U;
    uint8_t port1 = 0U;
    const int interrupt_level = gpio_get_level(board::kIoExpanderInterrupt);
...
esp_err_t Tca9555PowerKey::EnterPowerSave() {
    esp_err_t status = gpio_wakeup_enable(board::kIoExpanderInterrupt, GPIO_INTR_LOW_LEVEL);
    return status == ESP_OK ? gpio_intr_enable(board::kIoExpanderInterrupt) : status;
}

esp_err_t Tca9555PowerKey::ExitPowerSave() {
    esp_err_t status = gpio_intr_disable(board::kIoExpanderInterrupt);
    esp_err_t wake_status = gpio_wakeup_disable(board::kIoExpanderInterrupt);
...
int32_t Tca9555PowerKey::GetInterruptGpio(button_driver_t*) { return board::kIoExpanderInterrupt; }
...
esp_err_t Tca9555PowerKey::DeleteDriver(button_driver_t* driver) {
    ...
    if (owner->isr_registered_) {
        (void)gpio_isr_handler_remove(board::kIoExpanderInterrupt);
...
void IRAM_ATTR Tca9555PowerKey::InterruptEntry(void*) {
    iot_button_power_save_wakeup_isr(static_cast<uint32_t>(board::kIoExpanderInterrupt));
}
