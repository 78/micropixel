// Graphics 1.5 Direct Surface, positive path through the SDK with a
// HostSurface (Host-owned buffers, no pinned memory needed): a double-buffered
// surface presents 60 full frames and every buffer comes back through
// kSurfaceReleased in presentation order. Frames are painted with Graphics 1.6
// raster rects when the Host has kernels, otherwise presented as they are.
// Exit codes 30..49 name the failed step.
...
bool FillFrame(bool raster_supported, micropixel::HostSurface& surface, uint32_t index, uint32_t frame) {
    if (!raster_supported) {
        return true;
    }
    return surface
        .Update(index,
                [&](micropixel::RasterDrawList& list) {
                    const int32_t band = static_cast<int32_t>(surface.buffer_height() / 4U);
                    const micropixel::Color colors[4] = {
                        micropixel::Color::Rgb(255U, 0U, 0U), micropixel::Color::Rgb(0U, 255U, 0U),
                        micropixel::Color::Rgb(0U, 0U, 255U), micropixel::Color::White()};
                    for (uint32_t row = 0U; row < 4U; ++row) {
                        const micropixel::Rect area{0, static_cast<int32_t>(row) * band,
                                                    static_cast<int32_t>(surface.buffer_width()), band};
                        if (!list.FillRect(area, colors[(row + frame) % 4U])) {
                            return;
                        }
                    }
                    // A half-transparent strip across the middle exercises the blend path.
                    const micropixel::Rect strip{0, band * 2 - band / 4, static_cast<int32_t>(surface.buffer_width()),
                                                 band / 2};
...
int main() {
    ...
    // does not declare: refused as unsupported through the SDK as well.
    auto guest = app.renderer().CreateGuestSurface(kBufferCount);
    if (guest || guest.error().code() != micropixel::ErrorCode::kUnsupported) {
        return 32;
    }

    auto created = app.renderer().CreateHostSurface(kBufferCount);
    if (!created) {
        app.log().Error("direct_surface_present: create failed");
        return 30;
    }
    micropixel::HostSurface surface = static_cast<micropixel::HostSurface&&>(*created);
    if (!surface.valid() || surface.width() != info.physical_width() || surface.height() != info.physical_height() ||
        ...
        surface.buffer_count() != kBufferCount || surface.direct_scanout() != info.direct_scanout() ||
    ...
    // A second surface of either kind must be refused while this one exists.
    if (app.renderer().CreateHostSurface(1U) || app.renderer().CreateGuestSurface(1U)) {
        return 34;
    }

    const bool raster_supported = info.raster_supported();
    if (raster_supported) {
        // A list on a busy or out-of-range buffer is refused on the SDK side.
        if (surface.Update(kBufferCount, [](micropixel::RasterDrawList&) { __builtin_trap(); }).has_value()) {
            return 46;
        }
    }
    ...
    while (released + 1U < kFrames) {
        ...
        if (presented < kFrames && surface.AcquireFree(index)) {
            ...
            if (!FillFrame(raster_supported, surface, index, presented)) {
                return 47;
            }
            ...
            if (raster_supported &&
                surface.Update(index, [](micropixel::RasterDrawList&) { __builtin_trap(); }).has_value()) {
                return 48;
            }
    ...
    auto second = app.renderer().CreateHostSurface(1U);
    if (!second) {
        return 43;
    }
    second->Reset();
