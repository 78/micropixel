#ifndef MICROPIXEL_PLATFORM_DRIVERS_INPUT_ESP_LCD_TOUCH_INPUT_HPP
#define MICROPIXEL_PLATFORM_DRIVERS_INPUT_ESP_LCD_TOUCH_INPUT_HPP
...
// InputBackend contract. The controller ISR only schedules fixed-capacity I2C
// work; all bus access and Guest/Host event delivery runs on the executor task.
class EspLcdTouchInput final : public device::InputBackend {
   public:
    EspLcdTouchInput(int32_t width, int32_t height);

    [[nodiscard]] esp_err_t Initialize(esp_lcd_touch_handle_t touch, common::I2cExecutor& executor);
